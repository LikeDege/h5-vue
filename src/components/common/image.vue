...
export default {
  ...
  computed: {
      imgUrl() {
        ...
        if (src) {
          let width = parseInt(this.imgWidth * 2 / window.devicePixelRatio),height = parseInt(this.imgHeight * 2 / window.devicePixelRatio);
      ...
      imgHeight() {
        return +this.options.width * (this.options.scale ? this.options.scale : 0.75);
      },
  },
};
</script>
